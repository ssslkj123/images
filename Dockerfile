...
# 安装基础的工具链
RUN yum install -y tmux net-tools qperf iperf3 vim wget lrzsz tree screen lsof tcpdump nmap telnet dos2unix unix2dos bash-completion OpenIPMI-tools sysstat iptraf ntp zlib-devel ncurses-devel openssl openssh-serve openssl-devel mysql-devel mysql supervisor git redis gcc gcc-c++ golang htop iperf iftop iproute less unzip zip tcpdump nmap yum-util nfs-utils iotop java-1.8.0-openjdk stress stress-ng fio && yum clean all
...
COPY ["docker-enterpoint.sh","check_pod_health.sh","/" ]
# 启动镜像默认运行挂起脚本
